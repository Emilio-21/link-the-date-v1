...
import { OLIVOS_SLOTS, OLIVOS_SECTIONS } from "@/app/templates/plantilla_olivos/content";
...
export const TEMPLATES = {
  ...
  plantilla_olivos: {
    ...
    name: "Olivos",
    description: "Editorial moderno: crema y azul empolvado, cada sección encabezada por una foto a sangre.",
    component: PlantillaOlivos,
    // Además del mosaico, las fotos encabezan cada sección: caben más.
    gallerySize: 10,
    supports: {
      dressCode: true,
      kidsPolicy: true,
    ...
    // Textos/fuentes editables por sección (ver content.js de la plantilla)
    contentSlots: OLIVOS_SLOTS,
    contentSections: OLIVOS_SECTIONS,
  },
};
...
// Slots de texto/fuente editables de una plantilla (vacío si no soporta).
export function getTemplateContentSlots(slug) {
  return getTemplate(slug).contentSlots || [];
}

// Secciones (en orden) para agrupar los slots en el formulario.
export function getTemplateContentSections(slug) {
  return getTemplate(slug).contentSections || [];
}
